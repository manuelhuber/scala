...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    var high: Tweet = elem
    foreach(t => if (t.retweets > high.retweets) high = t)
    high
  }
...
object GoogleVsApple {
  ...
  def filterForKeywords(tweets: TweetSet, keywords: List[String]): TweetSet = {
    TweetReader.allTweets.filter(tweet => keywords.exists(word => tweet.text.contains(word)))
  }

  lazy val googleTweets: TweetSet = filterForKeywords(TweetReader.allTweets, google)
  lazy val appleTweets: TweetSet = filterForKeywords(TweetReader.allTweets, apple)
